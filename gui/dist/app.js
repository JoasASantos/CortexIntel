...
function selectNode(id) {
  ...
  const iscore=n.attributes&&n.attributes.intel_score!=null?parseInt(n.attributes.intel_score):null;
  const ibreak=n.attributes&&n.attributes.intel_breakdown;
  const iColor=iscore==null?"#888":(iscore>=70?"#ef4444":iscore>=45?"#fb7185":iscore>=25?"#f59e0b":"#34d399");
  $("#ctxRisk").innerHTML =
    (iscore!=null&&!n.meta?`<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="font-size:26px;font-weight:700;color:${iColor};line-height:1">${iscore}</div><div style="font-size:10px;color:#8fa8c0"><div style="letter-spacing:0.5px">${esc(t2("iscore.label"))}</div>${ibreak?`<div class="muted">${esc(ibreak)}</div>`:""}</div></div>`:"")+
    `<span class="band ${band}">${band} · ${(n.risk||0).toFixed(2)}</span><div class="risk-bar"><span style="width:${Math.round((n.risk||0)*100)}%;background:${bandColor(band)}"></span></div>`+
...
$$(".nav li").forEach(li=>li.addEventListener("click",()=>{
  // "Sources" is a shortcut into Settings → Data Sources (uploading data is the
  // first action; surface it as a top-level menu item).
  if(li.dataset.view==="sources"){ showView("settings"); openSettingsTab("datasources"); $$(".nav li").forEach(x=>x.classList.toggle("active",x===li)); return; }
...
let gfPreset="all";
let gfActiveEdges=null; // Set of enabled edge types, or null(all)
// Colour per edge-type family for the legend (derived signals get a distinct hue).
function edgeTypeColor(tp){ if(tp==="predicted_link")return "#F59E0B"; if(tp==="similar_to")return "#C084FC"; if(tp==="co_located_with")return "#57D7E8"; if(tp==="co_active_with")return "#4ADE80"; if(tp==="co_communicated_with")return "#60A5FA"; if(tp==="same_selector_as")return "#F472B6"; if(/same_ip|same_device|shares_|belongs_to/.test(tp))return "#93C5FD"; return "rgba(148,163,184,0.7)"; }
function renderGraphFilters(){ const t=activeTab(); const kinds=[...new Set((t?.graph.nodes||[]).map(n=>n.kind))]; const w=$("#gfKinds"); if(!w)return; w.innerHTML="";
  ...
  const ew=$("#gfEdges"); if(ew){ ew.innerHTML="";
    const types=[...new Set((t?.graph.edges||[]).map(e=>e.type))].sort();
    types.forEach(tp=>{ const chip=el("div","gf-kind"+((gfActiveEdges&&!gfActiveEdges.has(tp))?" off":"")); const d=el("span","kdot"); d.style.cssText=`background:${edgeTypeColor(tp)};border-radius:1px;width:10px;height:3px`; chip.appendChild(d); chip.appendChild(el("span",null,String(tp).replace(/_/g," ")));
      chip.addEventListener("click",()=>{ if(!gfActiveEdges) gfActiveEdges=new Set(types); if(gfActiveEdges.has(tp))gfActiveEdges.delete(tp); else gfActiveEdges.add(tp); applyFilters(); renderGraphFilters(); }); ew.appendChild(chip); });
  }
}
function applyFilters(){ if(!cy)return; const t=activeTab(); if(!t)return;
  ...
  // Edge-type filter: hide edges whose type is deselected (and any edge whose
  // endpoints are hidden).
  cy.edges().forEach(ed=>{ const tp=ed.data("type"); const typeOk = !gfActiveEdges || gfActiveEdges.has(tp);
    const ends = ed.source().style("display")!=="none" && ed.target().style("display")!=="none";
    ed.style("display", (typeOk&&ends)?"element":"none"); });
